...
import 'dart:ffi';
import 'package:embedded_serialport/src/flutter_rust.dart';
...
class Serial {
  ...
  SerialReadEvent read(int timeout, [int len = 256]) {
    _checkStatus();
  ...
  ///Sets the timeout
  ///
  ///Timeout can be between 0 and 25.5 seconds, with a resolution of 0.1 seconds.
  /// ```dart
  /// var serial = Serial('/dev/ttyUSB0', Baudrate.b115200);
  /// serial.timeout(2); // sets timeout to 2 seconds
  /// ```
  void timeout(double vtime) {
    _checkStatus();
    _checkError(_nativeSerialSetVtime(_serialHandle, vtime));
  }

  ///get the available port(s)
  ///
  /// ```dart
  /// List<String> ports = Serial.getPorts();
  /// ```
  ///
  static List<String> getPorts() {
    final ports = portsFromRust();
    final portsString = ports.toDartString();
    freePortsString(ports);
